defmodule InClassExerciseWeb.AreaController do
  ...
  def index(conn, _params) do
    areas = Developers.list_areas()
    render(conn, "index.html", areas: areas)
  end

  def new(conn, _params) do
    changeset = Developers.change_area(%Area{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"area" => area_params}) do
    case Developers.create_area(area_params) do
      {:ok, area} ->
        conn
  ...
  def update(conn, %{"id" => id, "area" => area_params}) do
    ...
    case Developers.update_area(area, area_params) do
      {:ok, area} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    area = Developers.get_area!(id)
    {:ok, _area} = Developers.delete_area(area)
